      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            ...
            within current_path do
              cmd = %{#{SSHKit.config.command_map[:rake]} RACK_ENV=#{rails_env} RAILS_ENV=#{rails_env} #{fetch(:resque_extra_env)} QUEUE="#{queue}" PIDFILE=#{pid} BACKGROUND=yes #{"VERBOSE=1 " if fetch(:resque_verbose)}INTERVAL=#{fetch(:interval)} #{"environment " if fetch(:resque_environment_task)}resque:work #{output_redirection}}
              if fetch(:resque_nohup)
                execute :nohup, cmd
              else
                execute cmd
              end
  ...
  namespace :scheduler do
    ...
    task :start do
      on roles :resque_scheduler do
        ...
        pid = "#{fetch(:resque_pid_path)}/scheduler.pid"
        within current_path do
          cmd = %{#{SSHKit.config.command_map[:rake]} RACK_ENV=#{rails_env} RAILS_ENV=#{rails_env} #{fetch(:resque_extra_env)} PIDFILE=#{pid} BACKGROUND=yes #{"VERBOSE=1 " if fetch(:resque_verbose)}MUTE=1 #{"DYNAMIC_SCHEDULE=yes " if fetch(:resque_dynamic_schedule)}#{"environment " if fetch(:resque_environment_task)}resque:scheduler #{output_redirection}}
          if fetch(:resque_nohup)
            execute :nohup, cmd
          else
            execute cmd
